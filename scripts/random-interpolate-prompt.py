...
def run(
        ...
        prompt_2 = "beautiful druid princess",
        gpu = 0, # id of the gpu to run on
        name = 'druid', # name of this project, for the output directory
        rootdir = '/home/darth/dreams',
        ...
        num_inference_steps = 15, # more (e.g. 100, 200 etc) can create slightly better images
    ...
    text_input = pipe.tokenizer(prompt, padding="max_length", max_length=pipe.tokenizer.model_max_length, truncation=True, return_tensors="pt")
    text_input_2 = pipe.tokenizer(prompt_2, padding="max_length", max_length=pipe.tokenizer.model_max_length, truncation=True, return_tensors="pt")
    
    cond_embeddings = pipe.text_encoder(text_input.input_ids.to(torch_device))[0] # shape [1, 77, 768]
    cond_embeddings_2 = pipe.text_encoder(text_input_2.input_ids.to(torch_device))[0] # shape [1, 77, 768]
    ...
    while frame_index < max_frames:
        ...
        for i, t in enumerate(np.linspace(0, 1, num_steps)):
            ...
            cond_final = slerp(float(t), cond_embeddings.detach(), cond_embeddings_2.detach())

            print("dreaming... ", frame_index)
            with autocast("cuda"):
                # image = diffuse(pipe, cond_embeddings, init, num_inference_steps, guidance_scale, eta)
                image = diffuse(pipe, cond_final, init1, num_inference_steps, guidance_scale, eta)
